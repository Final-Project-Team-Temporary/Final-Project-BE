...
- 🚫 **TDD 강제**: 동작이 바뀌는 기능 추가·개선은 테스트를 먼저 작성하고 실패를 확인한 뒤 구현한다. 실패하는 테스트 없이 기능 코드를 먼저 작성하지 않는다. (예외는 6장 참조)
...
## 6. 작업 워크플로우 — TDD 강제 (Plan → Red → Green → Refactor)

**모든 기능 추가·개선 작업은 TDD로 진행한다. 테스트를 먼저 작성하지 않고 기능 코드를 작성하지 않는다.** (5장 절대 규칙 참조)

1. **Plan(계획)**: 관련 도메인 패키지와 컨벤션을 먼저 읽고, 변경 범위·영향 파일·필요한 `ErrorStatus`/DTO/엔드포인트와 **검증할 동작(테스트 케이스 목록)**을 정리한다. 모호하면 추측하지 말고 질문한다.
2. **🔴 Red(실패 테스트 먼저)**: 추가/변경할 동작을 검증하는 테스트를 **먼저** 작성한다(7장 규약). 그 테스트를 실행해 **의도한 이유로 실패(컴파일 실패 포함)하는 것을 반드시 확인하고 그 결과를 보고**한다. 이 단계를 건너뛰지 않는다.
3. **🟢 Green(통과시키는 최소 구현)**: 방금 작성한 테스트를 통과시킬 **최소한의** 기능 코드를 기존 패턴(4장)대로 작성한다. `./gradlew test --tests "..."`로 통과를 확인한다.
4. **🔵 Refactor(정리)**: 테스트가 통과하는 상태를 유지하며 중복 제거·네이밍·구조를 정리한다. 변경 후 다시 테스트를 돌려 그린을 유지한다.
5. **Report**: 추가한 테스트, 변경 파일, 추가 엔드포인트, 새 `ErrorStatus`, 테스트 실행 결과(있는 그대로), 남은 TODO를 요약한다.

**예외(테스트를 먼저 안 써도 되는 경우)**: 순수 설정/문서/주석 변경, 의존성 버전 업, 로그 문구 수정 등 **동작(로직)이 바뀌지 않는** 변경. 이 경우 5장 규칙이 아님을 한 줄로 밝히고 진행한다. 동작이 바뀌는지 애매하면 TDD를 적용한다.

**환경 제약**: 통합/Testcontainers 테스트는 Docker가 필요하다. Docker가 없으면 **순수 단위 테스트(`POJOTestSupport`) 수준에서라도 Red→Green을 지키고**, 통합 테스트는 작성만 해 두되 "Docker 환경에서 실행 필요"로 명시 보고한다. 테스트 자체를 생략하지 않는다.

자주 쓰는 작업은 슬래시 커맨드로 표준화되어 있다: `/feature`(TDD 루프), `/new-domain`, `/review-diff`. ( `.claude/commands/` 참고 )
